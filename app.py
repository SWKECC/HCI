...
def get_frame():
    # 图片上传保存的路径
    with open(r'./templates/bg.png', 'rb') as f:
        image = f.read()
...
def get_frame_m():
    # 图片上传保存的路径
    with open(r'./templates/micro.png', 'rb') as f:
        image = f.read()
        resp = Response(image, mimetype="image/png")
        return resp
...
@app.route('/load_this_logs', methods=['POST'])
def load_this_logs():
    name = request.form['thisname']
    tmp = np.loadtxt("./logs/{}.csv".format(name), dtype=np.str, delimiter=",")
    res = tmp.reshape(-1)
    return res.tolist()
...
def load_sign_logs():
    tmp = np.loadtxt("./logs/logs.csv", dtype=np.str, delimiter=",")
    res = tmp.reshape(-1)
    return res.tolist()
...
def save_sign_logs():
    ...
    logs = np.array(logsinfo)
    work_id = int(logs[-1])
    logs = logs[:-1].reshape(-1, 3)
    np.savetxt("./logs/logs.csv", logs, delimiter=",", fmt="%s")
    if work_id != -1:
        name = logs[work_id][0]
        sign_type = logs[work_id][1]
        current_date = datetime.date.today().strftime('%Y-%m-%d ')
        sign_time = current_date+logs[work_id][2]
        this_log = np.array([sign_time, sign_type]).reshape(1, 2)
        with open("./logs/{}.csv".format(name), 'a') as f:
            np.savetxt(f, this_log, delimiter=",", fmt="%s")
        f.close()
    return "Done"
